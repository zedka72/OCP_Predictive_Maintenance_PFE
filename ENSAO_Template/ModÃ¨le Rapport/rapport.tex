...
\begin{itemize}
    \item \textbf{Co\^uts importants} : Mobilisation d'équipes de nuit/week-end, surtemps, urgences d'approvisionnement
    \item \textbf{Impact production} : Arr\^et non planifié de la chaîne de production, non-respect des livraisons
...
\begin{itemize}
    \item \textbf{Interventions inutiles} : L'équipement peut \^etre en bon état, mais l'intervention est quand m\^eme réalisée
...
Le site de Safi dispose d'un réseau capteurs étendu mais ces données sont actuellement collectées dans une base de données sans analyse prédictive. Les signaux de dégradation progressive sont ignorés, m\^eme lorsqu'ils auraient pu permettre une intervention anticipée.
...
\hline
Random Forest / XGBoost & Moyenne-\'Elevée & Excellent & Bonne \\
\hline
...
Entra\^{\i}ner un modèle de Machine Learning capable de :
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Compétences} : \'Equipe composée d'étudiants (pas de data engineers expérimentés)
\end{itemize}
...
\begin{itemize}
    \item \textbf{\'Ecosystème mature} : Les bibliothèques pandas, NumPy, Scikit-learn et XGBoost offrent des outils de classe mondiale pour le ML
    \item \textbf{Communauté} : Large communauté de data scientists avec abondance d'exemples, tutoriels, et bonnes pratiques
    \item \textbf{Rapidité développement} : Syntaxe intuitive et dynamique permettant une imp\'lémentation rapide
...
o\`u $l$ est la fonction de perte, $\Omega$ est le terme de régularisation (L1/L2), $m$ est le nombre d'arbres, et $y_i$, $\hat{y}_i$ sont les vraies et prédites valeurs.
...
\begin{itemize}
    \item \textbf{Robustesse} : Base de données open-source extr\^emement stable, utilisée en production par Fortune 500
    \item \textbf{Caractéristiques avancées} : Types de données avancés (JSON, UUID), vues matérialisees, foreign keys
    \item \textbf{Performance} : Optimiser élab\'orée, indexation B-tree, support des parallélisation de requêtes
...
\begin{itemize}
    ...
    \item \textbf{Adoption} : TR\`ES large adoption en entreprise, formation déjà disponible en interne
\end{itemize}
...
\hline
Temps entra\^{\i}n. & Moyen & Rapide & Très rapide & Lent \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \texttt{temp\_diff} : \'Ecart thermique lié aux conditions d'opération
...
Un modèle de baseline est entra\^{\i}né pour comparaison :
...
Le modèle XGBoost principal est entra\^{\i}né avec hyperparamètres optimisés :
